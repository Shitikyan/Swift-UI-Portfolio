...
public struct ContactView: View {
  ...
  public var body: some View {
    ...
    .navigationTitle("Hovhannes Shitikyan")
    .toolbar {
#if os(macOS)
...
//  }
    ...
    var header: some View {
      HStack(alignment: .top) {
        if let staticURL = URL(string: "https://media.licdn.com/dms/image/D4D03AQGL9l5zrkW07w/profile-displayphoto-shrink_400_400/0/1674064206745?e=1708560000&v=beta&t=wQZ8xWfU2Bp0cSst8bq9I8zRhfTE6AvPLrOOUD5BMjc") {
          AvatarView(url: staticURL)
            .containerRelativeFrame(.horizontal) { width, _ in
              min(width, maxContentWidth) / 3
            }
        }
  ...
  var details: some View {
    VStack(alignment: .leading) {
      WithViewStore(store, observe: \.contact?.name) { viewStore in
        ...
        let placeholder = "Hovhannes Shitikyan"
        let text = value ?? placeholder

        Text("Hovhannes Shitikyan")
          .font(.title)
